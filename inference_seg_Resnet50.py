...
class ResNetEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.maxpool(x)
        x = self.layer1(x); features.append(x) # 1/4, 256
        x = self.layer2(x); features.append(x) # 1/8, 512
        x = self.layer3(x); features.append(x) # 1/16, 1024
        x = self.layer4(x); features.append(x) # 1/32, 2048
        return features
...
def load_contour_model(encoder_path, decoder_path, device):
    # 使用修改后的 UNetResNet50
    model = UNetResNet50(num_classes=1, pretrained=False)
...
def main():
    ...
    # 加载模型（注意：这里使用了 UNetResNet50，但加载的权重文件名仍可保持不变）
